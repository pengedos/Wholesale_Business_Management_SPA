...
const catalogueSkuCount = document.getElementById("catalogueSkuCount");
const catalogueCategoryCount = document.getElementById("catalogueCategoryCount");
const printCatalogueButton = document.getElementById("printCatalogueButton");
const copyCatalogueButton = document.getElementById("copyCatalogueButton");
...
function formatMetricPrice(value) {
  const converted = Math.round(Number(value || 0) * currencyRates[currentCurrency]);
  return new Intl.NumberFormat(currentCurrency === "PHP" ? "en-PH" : "en-US", {
    style: "currency",
    currency: currentCurrency,
...
function getVisibleProducts() {
  return products.filter((product) => {
    ...
    const searchable = `${product.name} ${product.origin} ${product.category} ${product.sku} ${product.packSize} ${product.moq || ""} ${product.leadTime || ""} ${product.caseConfig || ""} ${product.buyerUse || ""}`.toLowerCase();
    ...
    return `
      <article class="product-card" data-category="${product.category}">
        ...
        <div class="product-body">
          ...
          <div class="product-use"><i class="fa-solid fa-store"></i> Best for: ${product.buyerUse || "Wholesale buyers"}</div>
          ...
          <div class="product-extra"><span><i class="fa-solid fa-box-open"></i> ${product.caseConfig || "Wholesale pack"}</span></div>
...
function renderCatalogueSummary() {
  if (catalogueSkuCount) {
    catalogueSkuCount.textContent = String(products.length);
  }

  if (catalogueCategoryCount) {
    const categories = new Set(products.map((product) => product.category));
    catalogueCategoryCount.textContent = String(categories.size);
  }
}

function renderCatalogueSheet() {
  if (!catalogueTableBody) return;

  catalogueTableBody.innerHTML = products.map((product) => {
    const stockState = getStockState(product.stock);
    return `
      <tr>
        ...
        <td>${product.packSize}<span>MOQ: ${product.moq || "Request"}</span></td>
        <td>${product.leadTime || "Confirm"}</td>
        <td><span class="mini-stock ${stockState.className}">${stockState.label} • ${product.stock} units</span></td>
...
function getCatalogueText() {
  const lines = [
    "Sikat Araw Trading Corp. - Product Catalogue",
    "SKU | Product | Category | Pack / MOQ | Lead Time | Storage | Est. Price",
    ...products.map((product) => [
      product.sku,
      product.name,
      getCategoryLabel(product.category),
      `${product.packSize} / MOQ: ${product.moq || "Request"}`,
      product.leadTime || "Confirm",
      product.storage,
...
function printCatalogue() {
  ...
  window.print();
  window.setTimeout(() => document.body.classList.remove("printing-catalogue"), 500);
}

async function copyCatalogueList() {
  try {
    await navigator.clipboard.writeText(getCatalogueText());
    showToast("Product catalogue list copied");
  } catch (error) {
    console.warn("Clipboard copy failed", error);
    showToast("Unable to copy catalogue list");
  }
}
  ...
  if (adminKpiGrid) {
    adminKpiGrid.innerHTML = `
      ...
      <article class="admin-kpi-card">
        ...
        <small><span class="metric-money">${formatMetricPrice(metrics.averageValue)}</span> average quote value</small>
      </article>
      <article class="admin-kpi-card">
...
function renderBackendStatus() {
  ...
  const storageLabel = status.remoteReady ? "Supabase" : "Local fallback";
  const remoteLabel = status.remoteReady ? "Ready" : "Not connected";
  const remoteNote = `Configured: ${status.remoteConfigured ? "Yes" : "No"} • SDK: ${status.sdkAvailable ? "Loaded" : "Not loaded"}`;
  const statusCards = `
    ...
    <article class="backend-status-card">
      ...
      <small>${escapeHTML(status.message || "Using browser localStorage fallback until Supabase is connected.")}</small>
    </article>
    <article class="backend-status-card">
    ...
    <article class="backend-status-card ${status.remoteReady ? "ready" : "warning"}">
      ...
      <small>${escapeHTML(remoteNote)}</small>
    </article>
  `;
...
initializeSwitchers();
renderBackendStatus();
if (printCatalogueButton) {
  printCatalogueButton.addEventListener("click", printCatalogue);
}

if (copyCatalogueButton) {
  copyCatalogueButton.addEventListener("click", copyCatalogueList);
}
